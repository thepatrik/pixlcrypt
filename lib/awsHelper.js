...
module.exports.getPresignedUrl = (bucket, key, op, exp) => {
    return getSignedUrl(bucket, key, op, exp);
};

module.exports.getPresignedUrlAsync = (bucket, key, op, exp) => {
    return getSignedUrl(bucket, key, op, exp, true);
};

function getSignedUrl(bucket, key, op, exp, async) {
    let s3 = new AWS.S3({
        signatureVersion: "v4"
    });
    ...
    if (async) {
        return new Promise((resolve, reject) => {
            s3.getSignedUrl(operation, opts, (err, url) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(url);
    ...
    return s3.getSignedUrl(operation, opts);
}
