...
router.get("/presign", (req, res, next) => {
    ...
    awsHelper.getPresignedUrlAsync(o.bucket, o.key, req.query.operation).then(presigned => {
        res.send({
            url: url,
            presigned: presigned
        });
...
router.post("/presign", (req, res, next) => {
    ...
    async.mapLimit(urls.split(","), 10, (url, done) => {
        ...
        if (!awsHelper.isS3Url(url) || !awsHelper.isPresignPermitted(url, req.user.email)) {
            ...
            const o = awsHelper.toS3Obj(url);
            awsHelper.getPresignedUrlAsync(o.bucket, o.key, req.body.operation).then(presigned => {
                done(null, {
                    url: url,
                    presigned: presigned
                });
